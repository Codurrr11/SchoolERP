<?php
// modules/school/leads/index.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        try {
            $session_id = $_SESSION['academic_session_id'] ?? 1;

            $stmt = $pdo->prepare("
                INSERT INTO leads (
                    school_id, session_id, class_id, source, referred_by, first_name, last_name, email, mobile_no, gender, dob,
                    mother_name, mother_qualification, mother_address, mother_occupation, mother_official_address, mother_income, mother_email, mother_mobile,
                    father_name, father_qualification, father_address, father_occupation, father_official_address, father_income, father_email, father_mobile,
                    nationality, religion, category, aadhar_no,
                    last_school_name, last_school_class, last_school_affiliation,
                    pincode, city, state, country, address,
                    status, remark, photo, created_by
                ) VALUES (
                    :school_id, :session_id, :class_id, :source, :referred_by, :first_name, :last_name, :email, :mobile_no, :gender, :dob,
                    :mother_name, :mother_qualification, :mother_address, :mother_occupation, :mother_official_address, :mother_income, :mother_email, :mother_mobile,
                    :father_name, :father_qualification, :father_address, :father_occupation, :father_official_address, :father_income, :father_email, :father_mobile,
            ...
            $_SESSION['flash_success'] = "Lead successfully created!";
        } catch (Exception $e) {
            $_SESSION['flash_error'] = "Failed to add lead: " . $e->getMessage();
        }
        header('Location: index.php');
        exit;
    }
    ...
    if ($action === 'edit') {
        ...
        // Check lead belongs to school
        $stmt = $pdo->prepare("SELECT photo FROM leads WHERE id = :id AND school_id = :school_id AND deleted_at IS NULL");
    ...
    if ($action === 'delete') {
        $id = intval($_POST['id'] ?? 0);
        try {
            $stmt = $pdo->prepare("UPDATE leads SET deleted_at = CURRENT_TIMESTAMP() WHERE id = :id AND school_id = :school_id");
            $stmt->execute([':id' => $id, ':school_id' => $school_id]);
            $_SESSION['flash_success'] = "Lead successfully deleted.";
        } catch (Exception $e) {
            $_SESSION['flash_error'] = "Failed to delete lead: " . $e->getMessage();
        }
        header('Location: index.php');
        exit;
    }
    ...
    if ($action === 'assign') {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE leads SET assigned_to = :assigned_to WHERE id = :id AND school_id = :school_id");
            $stmt->execute([':assigned_to' => $assigned_to, ':id' => $id, ':school_id' => $school_id]);
    ...
    if ($action === 'update_status') {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE leads SET status = :status, remark = :remark, scheduled_at = :scheduled_at WHERE id = :id AND school_id = :school_id");
            $stmt->execute([
                ':status' => $status,
                ':remark' => $remark,
    ...
    if ($action === 'convert') {
        ...
        // Fetch lead details
        $stmt = $pdo->prepare("SELECT * FROM leads WHERE id = :id AND school_id = :school_id AND deleted_at IS NULL");
        $stmt->execute([':id' => $id, ':school_id' => $school_id]);
        $lead = $stmt->fetch();
        ...
        try {
            ...
            $stmt_student = $pdo->prepare("
                INSERT INTO students (
                    ...
                    father_name, mother_name, mobile_no, dob, gender,
                    mother_address, father_address, status,
                    nationality, religion, category, aadhar_no, referred_by
                ) VALUES (
                    :school_id, :user_id, :session_id, :class_id, :first_name, :last_name,
                    :father_name, :mother_name, :mobile_no, :dob, :gender,
                    :mother_address, :father_address, 'active',
                    :nationality, :religion, :category, :aadhar_no, :referred_by
                )
            ");
            $stmt_student->execute([
                ...
                ':dob'            => $lead['dob'],
                ':gender'         => $lead['gender'],
                ':mother_address' => $lead['mother_address'] ?? '',
                ':father_address' => $lead['father_address'] ?? '',
...
// Fetch leads based on active status filter
...
$query_str = "
    ...
    WHERE l.school_id = :school_id AND l.deleted_at IS NULL
";
...
// Dynamic count metrics for top tabs
$stmt_counts = $pdo->prepare("
    ...
    GROUP BY status
");
$stmt_counts->execute([':school_id' => $school_id]);
...
<div class="row align-items-center mb-4 g-3">
    <div class="col-sm-6">
        <h2 class="mb-1 font-heading fw-extrabold">Leads & Inquiries</h2>
        <p class="text-xs text-muted mb-0">Record and follow up on prospective student inquiries and conversions.</p>
    </div>
</div>
...
<div class="row g-4">
    <div class="col-12">
        <div class="card-premium">
            <!-- Toolbar matching student directory style -->
            <div class="teacher-card-header d-flex flex-wrap justify-content-between align-items-center gap-3">
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    <button type="button" class="teacher-header-btn btn-accent" data-bs-toggle="modal" data-bs-target="#addLeadModal" title="Add Lead">
                        <i class="ph-bold ph-plus"></i>
                    </button>
                    <button type="button" class="teacher-header-btn btn-sky" title="Import Leads">
                        <i class="ph-bold ph-cloud-arrow-up"></i>
                    </button>
                    <button type="button" class="teacher-header-btn btn-sky" title="Export Leads">
                        <i class="ph-bold ph-cloud-arrow-down"></i>
                    </button>
                    <button type="button" class="teacher-header-btn btn-sky" id="filterToggleBtn" title="Filters">
                        <i class="ph-bold ph-funnel"></i>
                    </button>
                </div>

                <div class="d-flex align-items-center gap-3 w-100 w-sm-auto ms-auto justify-content-end">
                    <form method="GET" action="index.php" class="table-search-box m-0">
                        <i class="ph-light ph-magnifying-glass"></i>
                        <input type="text" name="search" placeholder="Search inquiries..." value="<?php echo sanitize($search_query); ?>" style="height: 34px; padding-top: 0; padding-bottom: 0;">
                    ...
                    <div class="teacher-total-badge bg-info text-white border-0 py-1.5 px-3 rounded-pill text-xxs font-heading fw-extrabold">
                        Total Leads: <?php echo $total_leads; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="px-4 pt-3 pb-2 border-bottom d-flex flex-wrap gap-2 align-items-center lead-filter-tabs">
                ...
                foreach ($counts as $t_name => $t_count):
                    ...
                    $active_class = ($active_tab === $t_name) ? 'active' : '';
                ?>
                    <a href="index.php?tab=<?php echo urlencode($t_name); ?>&search=<?php echo urlencode($search_query); ?>"
                        class="btn-filter <?php echo $tc_class; ?> <?php echo $active_class; ?>">
                        <?php echo $t_name; ?> (<?php echo $t_count; ?>)
                    </a>
                <?php endforeach; ?>
                <a href="index.php?tab=All&search=<?php echo urlencode($search_query); ?>"
                ...
                <div class="table-responsive">
                    <?php if (empty($leads)): ?>
                        <div class="p-5 text-center">
                            ...
                            <h5 class="fw-semibold mt-3">No inquiries found</h5>
                            <p class="text-xs text-muted mb-4">No prospects found matching selection criteria.</p>
                            <button type="button" class="btn-admin-action mx-auto" data-bs-toggle="modal" data-bs-target="#addLeadModal">
                                <i class="ph-bold ph-plus"></i> Add Lead
                            </button>
                        </div>
                    <?php else: ?>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center gap-2">
                                                ...
                                                <div class="d-flex flex-column">
                                                    <span class="student-name-link">Name: <?php echo sanitize($l['first_name'] . ' ' . $l['last_name']); ?></span>
                                                    <span class="text-xs text-muted">Father Name: <strong class="text-dark"><?php echo sanitize($l['father_name'] ?? '—'); ?></strong></span>
                                                    <span class="text-xs text-muted">Mother Name: <strong class="text-dark"><?php echo sanitize($l['mother_name'] ?? '—'); ?></strong></span>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="text-xs">
                                            <div class="d-flex flex-column">
                                                <span>
                                                    ...
                                                    <a href="https://wa.me/91<?php echo preg_replace('/\D/', '', $l['mobile_no']); ?>" target="_blank" class="whatsapp-icon" title="WhatsApp Chat">
                                                        <i class="ph-light ph-whatsapp-logo"></i>
                                                    </a>
                                                </span>
                                                <span class="text-muted">F. Mobile No.: <strong class="text-dark"><?php echo sanitize($l['father_mobile'] ?? '—'); ?></strong></span>
                                                <span class="text-muted">M. Mobile No.: <strong class="text-dark"><?php echo sanitize($l['mother_mobile'] ?? '—'); ?></strong></span>
                                            </div>
                                        </td>

                                        <!-- Address -->
                                        <td class="text-xs cell-truncate">
                                        ...
                                        <td class="text-xs">
                                            <div class="d-flex flex-column">
                                                ...
                                                <span class="text-muted"><?php echo date('d M, Y (h:i a)', strtotime($l['created_at'])); ?></span>
                                            </div>
                                        </td>
                                        ...
                                        <td class="text-xs text-muted">
                                            <?php if (!empty($l['scheduled_at'])): ?>
                                                <span class="fw-semibold text-dark"><i class="ph-light ph-clock me-1"></i><?php echo date('d M, Y h:i a', strtotime($l['scheduled_at'])); ?></span>
                                            <?php else: ?>
                                                —
                                            <?php endif; ?>
                                        </td>
                                        ...
                                        <td class="text-center">
                                            <div class="dropdown">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end shadow border-0">
                                                    ...
                                                    <li><a class="dropdown-item py-2 text-xs delete-lead-btn text-danger" href="#" data-id="<?php echo $l['id']; ?>"><i class="ph-light ph-trash me-2"></i> Delete</a></li>
                                                </ul>
                                            </div>
                                        </td>
                                    </tr>
    ...
    <div class="modal-dialog modal-xl modal-dialog-scrollable modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header modal-header-admin py-3 px-4">
                <h5 class="modal-title font-heading fw-bold text-dark" id="addLeadModalLabel">Add Lead</h5>
            ...
            <form action="index.php" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="source" class="form-control-admin">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="referred_by" class="form-control-admin" placeholder="Referred By">
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        <div class="col-md-4">
                            ...
                            <input type="text" name="first_name" class="form-control-admin" required placeholder="First Name">
                        </div>
                        <div class="col-md-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="email" name="email" class="form-control-admin" placeholder="Email Address">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label-admin">Mobile no. <span class="text-danger">*</span></label>
                            <input type="text" name="mobile_no" class="form-control-admin" required placeholder="Mobile no.">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label-admin d-block">Gender</label>
                            <div class="form-check form-check-inline mt-2">
                                <input class="form-check-input" type="radio" name="gender" id="add_gender_male" value="male" checked>
                                <label class="form-check-label text-xs" for="add_gender_male">Male</label>
                            </div>
                            <div class="form-check form-check-inline mt-2">
                                <input class="form-check-input" type="radio" name="gender" id="add_gender_female" value="female">
                                <label class="form-check-label text-xs" for="add_gender_female">Female</label>
                            </div>
                            <div class="form-check form-check-inline mt-2">
                                <input class="form-check-input" type="radio" name="gender" id="add_gender_other" value="other">
                                <label class="form-check-label text-xs" for="add_gender_other">Other</label>
                            </div>
                        </div>
                        <div class="col-md-4">
                        ...
                        <table class="table table-bordered table-sm align-middle text-xs mb-0">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td><input type="text" name="mother_name" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Mother's Name"></td>
                                    <td><input type="text" name="father_name" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Father's Name"></td>
                                </tr>
                                <tr>
                                    <td><strong>Qualification</strong></td>
                                    <td><input type="text" name="mother_qualification" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Qualification"></td>
                                    <td><input type="text" name="father_qualification" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Qualification"></td>
                                </tr>
                                <tr>
                                    <td><strong>Residential Address</strong></td>
                                    <td><input type="text" name="mother_address" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Residential Address"></td>
                                    <td><input type="text" name="father_address" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Residential Address"></td>
                                </tr>
                                <tr>
                                    <td><strong>Occupation</strong></td>
                                    <td><input type="text" name="mother_occupation" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Occupation"></td>
                                    <td><input type="text" name="father_occupation" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Occupation"></td>
                                </tr>
                                <tr>
                                    <td><strong>Official Address</strong></td>
                                    <td><input type="text" name="mother_official_address" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Official Address"></td>
                                    <td><input type="text" name="father_official_address" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Official Address"></td>
                                </tr>
                                <tr>
                                    <td><strong>Annual Income</strong></td>
                                    <td><input type="text" name="mother_income" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Annual Income"></td>
                                    <td><input type="text" name="father_income" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Annual Income"></td>
                                </tr>
                                <tr>
                                    <td><strong>Email Address</strong></td>
                                    <td><input type="email" name="mother_email" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Email Address"></td>
                                    <td><input type="email" name="father_email" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Email Address"></td>
                                </tr>
                                <tr>
                                    <td><strong>Mobile No.</strong></td>
                                    <td><input type="text" name="mother_mobile" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Mobile No."></td>
                                    <td><input type="text" name="father_mobile" class="form-control form-control-sm border-0 bg-light-focus" placeholder="Mobile No."></td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-4">
                            ...
                            <input type="text" name="last_school_class" class="form-control-admin" placeholder="Attended Classes">
                        </div>
                        <div class="col-md-4">
                            <label class="form-label-admin">Last School Affiliated to</label>
                            <input type="text" name="last_school_affiliation" class="form-control-admin" placeholder="Affiliated To">
                        </div>
                    </div>
                    ...
                    <div class="row g-3 mb-4">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" name="country" class="form-control-admin" value="India" readonly>
                        </div>
                        <div class="col-md-12">
                            <label class="form-label-admin">Full Address</label>
                            <textarea name="address" class="form-control-admin" rows="2" placeholder="Full Address"></textarea>
                        </div>
                    </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="status" class="form-control-admin" required>
                                <?php foreach ($counts as $t_name => $t_count): ?>
                                    <option value="<?php echo $t_name; ?>" <?php echo $t_name === 'Interested' ? 'selected' : ''; ?>><?php echo $t_name; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-4">
                            <label class="form-label-admin">Remark</label>
                            <input type="text" name="remark" class="form-control-admin" placeholder="Remark">
                        </div>
                        <div class="col-md-4">
                ...
                <div class="modal-footer modal-footer-admin p-3">
                    ...
                    <button type="submit" class="btn btn-primary font-heading px-4">Save</button>
                </div>
            </form>
        </div>
    </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <table class="table table-sm table-borderless mb-0">
                            <tr>
                                <td style="width:120px;" class="text-muted">Mobile No.</td>
                                <td class="fw-semibold" id="view_mobile"></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td id="view_gender" class="text-capitalize"></td>
                            </tr>
                            <tr>
                    ...
                    <div class="col-md-6">
                        ...
                        <table class="table table-sm table-borderless mb-0">
                            <tr>
                                <td style="width:120px;" class="text-muted">Address</td>
                    ...
                    <div class="col-12 mt-3">
                        ...
                        <div class="row g-3">
                            <div class="col-sm-6 border-end">
                                ...
                                <table class="table table-sm table-borderless mb-0">
                                    <tr>
                                        <td style="width:120px;" class="text-muted">Name</td>
                                    ...
                                    <tr>
                                        ...
                                        <td id="view_father_qualification"></td>
                                    </tr>
                                    <tr>
                                        <td class="text-muted">Occupation</td>
                                        <td id="view_father_occupation"></td>
                                    </tr>
                                </table>
                            </div>
                            <div class="col-sm-6">
                                ...
                                <table class="table table-sm table-borderless mb-0">
                                    ...
                                    <tr>
                                        <td class="text-muted">Qualification</td>
                                        <td id="view_mother_qualification"></td>
                                    </tr>
                                    <tr>
                                        <td class="text-muted">Occupation</td>
                                        <td id="view_mother_occupation"></td>
                                    </tr>
                                </table>
                            </div>
                        </div>
                    ...
                    <div class="col-12 mt-3">
                        ...
                        <table class="table table-sm table-borderless mb-0">
                            <tr>
                                <td style="width:140px;" class="text-muted">Created By</td>
                                <td id="view_created_by"></td>
                            </tr>
                            <tr>
                                <td class="text-muted">Scheduled Follow Up</td>
                                <td id="view_scheduled_at" class="fw-semibold"></td>
                            </tr>
                            <tr>
                                <td class="text-muted">Remark</td>
                                <td id="view_remark" class="text-dark bg-light p-2 rounded"></td>
                            </tr>
                        </table>
                    </div>
                </div>
...
<!-- Forms & AJAX handlers -->
<form id="deleteLeadForm" action="index.php" method="POST" style="display:none;">
    <input type="hidden" name="csrf_token" value="<?php echo $csrf_token; ?>">
    <input type="hidden" name="action" value="delete">
    <input type="hidden" name="id" id="delete_id">
</form>

<form id="convertLeadForm" action="index.php" method="POST" style="display:none;">
    <input type="hidden" name="csrf_token" value="<?php echo $csrf_token; ?>">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.view-lead-btn').forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                fetch('index.php?get_lead_details=1&id=' + id)
                    .then(res => res.json())
                    .then(res => {
                        if (res.success) {
                            const data = res.data;
        ...
        document.querySelectorAll('.delete-lead-btn').forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                Swal.fire({
                    ...
                    text: 'Are you sure you want to permanently delete this prospective lead? This action cannot be undone.',
                    icon: 'warning',
